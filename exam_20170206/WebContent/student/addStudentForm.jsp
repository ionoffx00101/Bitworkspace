...
$(document).ready(function() {
  ...
  $("#userid").change(function(){
	  	$.ajax({ 
		  	 url:"<c:url value="/StudentService"/>",
		  	 type:"post",
		  	 dataType:"text",
		     data:{
			     userid:$("#userid").val(),
  	...
  	<%--3.1 단대를 클릭했을때 호출되는 메서드 id 속성의 값이 colno인 객체를 선택했을 때 서버로 요청 전송되는 부분 --%>
...
<body>
	<form action="StudentService" method="post">
		...
		<p>
			단과대학 <select id="colno" name="colno">
		...
		<p>
			학과 <select id="deptno" name="deptno">
		...
		<p>
			아이디 <input id="userid" name="userid" id="userid" type="text" />
			<%-- 사용 가능한 ID인지 아닌지 --%>
